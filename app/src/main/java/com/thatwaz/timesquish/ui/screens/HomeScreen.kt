...
fun HomeScreen(
    ...
    val hourlyPayFlow by viewModel.hourlyPayFlow.collectAsState(initial = 0.0)
    var hourlyPayInput by remember { mutableStateOf(hourlyPayFlow.toString()) }
    ...
    Column(
        ...
        // Only show hourly pay input if not clocked in
        if (!isClockedIn) {
            OutlinedTextField(
        ...
        Button(
            onClick = {
                if (isClockedIn) {
                    ...
                    val pay = hourlyPayInput.toDoubleOrNull()
                    if (pay != null) {
                        scope.launch {
                            viewModel.saveHourlyPay(pay) // Update DataStore
                            viewModel.clockIn(pay)        // Save into the Session
                        }
                    } else {
                        // Invalid input — fallback or skip
                    }
                }
            },
            modifier = Modifier
